...
# Custom boot mount check to prevent kernel problem
if ! findmnt /boot > /dev/null 2>&1; then
  echo "⚠️  /boot is not mounted."
  if gum confirm "Do you want to try mounting /boot now?"; then
    ...
    echo "❌ Aborting update. /boot must be mounted."
    exit 1
  fi
fi
